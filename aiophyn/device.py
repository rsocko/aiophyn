...
class Device:
    ...
    async def get_water_usage_events(
        self,
        device_id: str,
        from_datetime: Union[datetime, int, None] = None,
        to_datetime: Union[datetime, int, None] = None,
        *,
        ...
        :param device_id: Unique identifier for the device
        :type device_id: str
        :param from_datetime: Start of time range, datetime or millisecond epoch
        :type from_datetime: datetime or int
        :param to_datetime: End of time range, datetime or millisecond epoch
        :type to_datetime: datetime or int
        :param from_ts: Alternative start in milliseconds (upstream keyword)
        :type from_ts: int
        :param to_ts: Alternative end in milliseconds (upstream keyword)
        :type to_ts: int
        :raises TypeError: A bound is missing, duplicated, or has an invalid type.
        ...
        .. note::
            ...
            Datetime objects are converted internally; integer timestamps pass
            through unchanged. Supply each bound using only one keyword.
        """
        def timestamp_param(
            value: Union[datetime, int, None], timestamp: Optional[int], name: str
        ) -> Union[str, int]:
            if value is not None and timestamp is not None:
                raise TypeError(f"Specify only one {name} bound")
            if timestamp is not None:
                if isinstance(timestamp, bool) or not isinstance(timestamp, int):
                    raise TypeError(f"{name}_ts must be an integer millisecond timestamp")
                return timestamp
            if isinstance(value, datetime):
                return str(int(value.timestamp() * 1000))
            if isinstance(value, int) and not isinstance(value, bool):
                return value
            raise TypeError(f"{name} bound must be a datetime or integer millisecond timestamp")

        params = {
            "device_id": device_id,
            "from_ts": timestamp_param(from_datetime, from_ts, "from"),
            "to_ts": timestamp_param(to_datetime, to_ts, "to"),
        }
