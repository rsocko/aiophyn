...
class TestWaterUsageEvents:
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize("keyword_bounds", [False, True])
    async def test_upstream_timestamp_bounds(self, device, mock_request, keyword_bounds):
        mock_request.return_value = SAMPLE_WATER_USAGE_EVENTS
        start, end = 0, 1771948800000
        if keyword_bounds:
            result = await device.get_water_usage_events(
                "DEVICE123", from_ts=start, to_ts=end
            )
        else:
            result = await device.get_water_usage_events("DEVICE123", start, end)

        mock_request.assert_awaited_once_with(
            "get",
            f"{API_BASE}/water-usage-events",
            params={"device_id": "DEVICE123", "from_ts": start, "to_ts": end},
        )
        assert result is SAMPLE_WATER_USAGE_EVENTS

    @pytest.mark.asyncio
    async def test_datetime_keywords(self, device, mock_request):
        await device.get_water_usage_events(
            "DEVICE123",
            from_datetime=datetime(2026, 1, 1, tzinfo=timezone.utc),
            to_datetime=datetime(2026, 1, 2, tzinfo=timezone.utc),
        )
        mock_request.assert_awaited_once_with(
            "get",
            f"{API_BASE}/water-usage-events",
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            {"from_ts": 0},
            {"to_ts": 1},
            {"from_datetime": 0, "from_ts": 0, "to_ts": 1},
            {"from_ts": 0, "to_datetime": 1, "to_ts": 1},
            {"from_ts": "0", "to_ts": 1},
            {"from_ts": 0, "to_ts": 1.5},
            {"from_datetime": True, "to_ts": 1},
            {"from_ts": 0, "to_ts": False},
        ],
    )
    async def test_invalid_bounds(self, device, mock_request, bounds):
        with pytest.raises(TypeError):
            await device.get_water_usage_events("DEVICE123", **bounds)
